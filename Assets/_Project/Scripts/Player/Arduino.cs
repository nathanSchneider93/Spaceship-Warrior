...
public class Giroscopio : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!dataString.Equals("NOT OPEN"))
        {
            ...
            float gx = int.Parse(dataRaw[3]) * gyro_normalizer_factor;
            float gy = int.Parse(dataRaw[4]) * gyro_normalizer_factor;
            float gz = int.Parse(dataRaw[5]) * gyro_normalizer_factor;
